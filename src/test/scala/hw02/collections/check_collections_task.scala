...
import hw02.collections.task_collections._
...
class check_collections_task extends AnyFlatSpec {
  ...
  "check intersectionAuto" should "ok" in {
    ...
    assert(
      intersectionAuto(dealerOne, dealerTwo) === Set(
        Auto("BMW", "i3"),
        Auto("Mazda", "X5")
      )
    )
  ...
  "check filterAllLeftDealerAutoWithoutRight" should "ok" in {
    ...
    val dealerTwo = Seq(Auto("BMW", "i3"), Auto("Mazda", "X5"))
    assert(
      filterAllLeftDealerAutoWithoutRight(dealerOne, dealerTwo) === Set.empty
    )

    val dealerOneSecond = Vector(Auto("BMW", "i3"), Auto("Mazda", "X5"))
    val dealerTwoSecond = Seq(Auto("BMW", "i3"))
    assert(
      filterAllLeftDealerAutoWithoutRight(
        dealerOneSecond,
        dealerTwoSecond
      ) === Set(Auto("Mazda", "X5"))
    )
